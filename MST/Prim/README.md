...
Given a **weighted**, **undirected**, and **connected** graph with `V` vertices and `E` edges, find the **sum of the weights** of the edges in the **Minimum Spanning Tree (MST)** using **Prim's Algorithm**.
...
Starting from any vertex, Prim's Algorithm repeatedly selects the minimum-weight edge that connects a visited vertex to an unvisited vertex.

The resulting Minimum Spanning Tree is shown below.
...
Only one spanning tree is possible, so Prim's Algorithm selects the only edge with weight `5`.
